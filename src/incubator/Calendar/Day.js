...
const Day = props => {
  ...
  const backgroundColor = !isToday(date) ? NO_COLOR : inactive ? INACTIVE_TODAY_BACKGROUND_COLOR : TODAY_BACKGROUND_COLOR;
  ...
  const animatedTextStyles = useAnimatedStyle(() => {
    return {
      color: withTiming(isSelected.value ? SELECTED_TEXT_COLOR : inactive ? showExtraDays ? INACTIVE_TEXT_COLOR : NO_COLOR : TEXT_COLOR, {
        duration: 100
      })
    };
  });
